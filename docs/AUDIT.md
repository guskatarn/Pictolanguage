...
2. ~~Traiter le sujet critique : rendre l'app fiable hors-ligne (embarquer les pictogrammes localement plutôt que de dépendre d'ARASAAC en ligne à chaque usage), et ajouter un export/import des profils pour ne pas perdre les données + corriger la perte silencieuse au dépassement de quota (§2.F.2).~~ ✅ **Terminé le 2026-09-03** (§2.F.1 et §2.F.2). Il reste, dans le même registre de fiabilité et désormais en tête de liste : **aucun test automatisé et aucune error boundary React** (§2.F) — prochain lot.
